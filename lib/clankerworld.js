...
const sanitizeContextId = (value) => String(value || '').trim().replace(/\s+/g, '_').slice(0, 80);
...
const buildFallbackContextId = (config) => {
    const platform = sanitizeContextId(config?.context?.platform || 'context') || 'context';
    const messageId = String(config?.context?.messageId || '').trim();
    const socialHint = Array.isArray(config?.metadata?.socialMediaUrls)
        ? String(config.metadata.socialMediaUrls[0]?.url || '').trim()
        : '';
    const seed = [platform, messageId, socialHint].filter(Boolean).join('|') || `${platform}|fallback`;
    return `auto_${platform}_${hashSeed(seed)}`;
};
...
export const maybeEnrichContextId = async (config, options = {}) => {
    ...
    const existingId = sanitizeContextId(config?.context?.id);
    ...
    if (!platform) {
        if (!existingId) {
            config.context.id = buildFallbackContextId(config);
            return { changed: true, reason: 'auto-fallback-platform-missing', id: config.context.id };
        }
        return { changed: false, reason: 'platform-missing' };
    }

    if (platform !== 'twitter') {
        if (!existingId) {
            config.context.id = buildFallbackContextId(config);
            return { changed: true, reason: 'auto-fallback-non-twitter', id: config.context.id };
        }
        return { changed: false, reason: 'platform-not-twitter' };
    }

    const currentId = existingId;
    if (/^\d+$/.test(currentId)) {
    ...
    try {
        ...
        if (!numericId) {
            if (!currentId) {
                config.context.id = buildFallbackContextId(config);
                return { changed: true, reason: 'auto-fallback-twitter-resolve-empty', username, id: config.context.id };
            }
        ...
        if (!currentId) {
            config.context.id = buildFallbackContextId(config);
            return { changed: true, reason: 'auto-fallback-twitter-resolve-failed', username, id: config.context.id };
        }
