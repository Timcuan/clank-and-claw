...
const buildAutoContextId = ({ platform, messageId, contextUrl, socialsInput }) => {
    const safePlatform = sanitizeContextId(platform || 'context') || 'context';
    const socialHint = socialsInput && typeof socialsInput === 'object'
        ? (socialsInput.x || socialsInput.twitter || socialsInput.farcaster || socialsInput.website || socialsInput.github || '')
        : '';

    const seed = [
        safePlatform,
        String(messageId || '').trim(),
        String(contextUrl || '').trim(),
        String(socialHint || '').trim()
    ].filter(Boolean).join('|');

    return `auto_${safePlatform}_${hashContextSeed(seed || `${safePlatform}|fallback`)}`;
};
...
const resolveContext = (contextInput, socialsInput, fallbackPlatform = 'twitter') => {
    ...
    const sanitizedUserId = sanitizeContextId(userId);
    context.messageId = messageId;
    context.id = sanitizedUserId || buildAutoContextId({
        platform: context.platform,
        messageId,
        contextUrl: contextUrlCandidate,
        socialsInput
    });
