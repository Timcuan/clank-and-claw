...
const parseTokenFees = (token) => {
    ...
    if (rawFees && typeof rawFees === 'object') {
        ...
        if (rawFees.total !== undefined) {
            ...
            if (parsed) return { type: 'static', ...parsed };
        }
...
export const loadTokenConfig = (filePath = 'token.json') => {
    ...
    const smartValidation = token.advanced?.smartValidation !== undefined
        ? parseBoolean(token.advanced.smartValidation)
        : (token.smartValidation !== undefined ? parseBoolean(token.smartValidation) : false);
