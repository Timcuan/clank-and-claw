...
const inferContextPlatformFromUrl = (input) => {
    ...
    try {
        const hostname = new URL(String(input).trim()).hostname.toLowerCase();
...
const resolveContext = (contextInput, socialsInput, fallbackPlatform = 'twitter') => {
    ...
    const urlInferredPlatform = inferContextPlatformFromUrl(contextInput?.url);
    const context = {
    ...
    const messageIdFromUrl = extractMessageId(contextInput?.url);
    ...
    if (messageIdFromUrl) {
        ...
        source = 'context-url';
        if (messageIdFromField && messageIdFromField !== messageIdFromUrl) {
            console.warn(`⚠️ \x1b[33mContext Mismatch:\x1b[0m messageId (${messageIdFromField}) differs from URL. Using URL-derived ID (${messageIdFromUrl}).`);
        }
    ...
    if (urlInferredPlatform && urlInferredPlatform !== explicitPlatform) {
        context.platform = urlInferredPlatform;
        console.warn(`⚠️ \x1b[33mContext Platform Override:\x1b[0m Using ${urlInferredPlatform} from context URL.`);
    }
...
export const loadTokenConfig = (filePath = 'token.json') => {
    ...
    // Support object format with 'total'
        ...
        if (token.fees.total) {
            ...
            fees = {
                type: 'static',
                clankerFee: parseIntSafe(token.fees.clankerFee, DEFAULT_STATIC_CLANKER_FEE_BPS),
                pairedFee: parseIntSafe(token.fees.pairedFee, DEFAULT_STATIC_PAIRED_FEE_BPS)
            };
        }
    }
