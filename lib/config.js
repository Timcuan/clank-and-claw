...
export const loadTokenConfig = (filePath = 'token.json') => {
    ...
    let msgId = extractMessageId(token.context?.url || token.context?.messageId);

    // Auto-scan socials if context missing/invalid
    if (!msgId && token.socials) {
        if (token.socials.x) {
            const twId = extractMessageId(token.socials.x);
            if (twId) { msgId = twId; context.platform = 'twitter'; }
        }
        if (!msgId && token.socials.farcaster) {
            const fcHash = extractMessageId(token.socials.farcaster);
            if (fcHash) { msgId = fcHash; context.platform = 'farcaster'; }
        }
    }

    // System Fallback (Env)
    if (!msgId && process.env.DEFAULT_CONTEXT_ID) {
        msgId = process.env.DEFAULT_CONTEXT_ID;
...
export const createConfigFromSession = (t, deployerAddress) => {
    // Smart Context Logic for Bot
    let context = {
        interface: 'Clanker SDK',
        platform: t.context?.platform || 'twitter'
    };

    let msgId = extractMessageId(t.context?.messageId || t.context?.url);

    // Auto-scan socials if context missing
    if (!msgId && t.socials) {
        if (t.socials.x) {
            const twId = extractMessageId(t.socials.x);
            if (twId) { msgId = twId; context.platform = 'twitter'; }
        }
        if (!msgId && t.socials.farcaster) {
            const fcHash = extractMessageId(t.socials.farcaster);
            if (fcHash) { msgId = fcHash; context.platform = 'farcaster'; }
        }
    }
