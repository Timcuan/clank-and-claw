...
export const loadTokenConfig = (filePath = 'token.json') => {
    ...
    let context = {
        ...
        platform: process.env.DEFAULT_PLATFORM || 'twitter',
        messageId: undefined
    };
    if (token.context) {
    ...
    // Token admin & spoofing
    const tokenAdmin = token.advanced?.admin || process.env.TOKEN_ADMIN || '0x0000000000000000000000000000000000000000';
    const spoofTo = token.advanced?.spoofTo || process.env.ADMIN_SPOOF;

    // Build rewards
    let rewards = { recipients: [] };
    const rewardRecipient = spoofTo || tokenAdmin;
    if (rewardRecipient && rewardRecipient !== '0x0000000000000000000000000000000000000000') {
    ...
    // Pool settings
    const startingTick = -230400;
    const poolPositions = POOL_POSITIONS.Standard || [
    ...
    return {
        ...
        _meta: {
            strictMode: false,
            highTax: fees.clankerFee + fees.pairedFee > 500,
            rewardRecipient,
