...
• Or type \`/cancel\` to stop
    ...
    if (hasData) {
        ...
        const missing = [];
        if (!t.name && !t.symbol) missing.push('name or symbol');

        return await sendMessage(chatId, `
📝 *Current Session:* ${t.symbol || 'New Token'}
        
${missing.length > 0 ? `❌ Missing: ${missing.join(', ')}` : '✅ Core config ready'}
...
Type: \`/go ${potentialSymbol[0]} "Name" 6%\`
