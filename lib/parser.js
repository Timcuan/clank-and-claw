...
export const parseSourceLink = (url) => {
    ...
    // Generic URL fallback (auto source context)
    if (/^https?:\/\//i.test(trimmed)) {
        let platform = 'website';
        try {
            const hostname = new URL(trimmed).hostname.toLowerCase();
            if (hostname.includes('t.me') || hostname.includes('telegram.')) platform = 'telegram';
            else if (hostname.includes('discord.gg') || hostname.includes('discord.com')) platform = 'discord';
 ...
 * Supports: 6%, 3% 3%, 600bps, 300 300, etc.
 */
 ...
 * 
 * Examples:
 * - "Deploy PEPE (Pepe Token) 6% https://x.com/user/status/123"
 * - "/go DOGE 'Dogecoin 2' 600bps"
 * - "Launch TOKEN with 6% fees"
 */
export const parseTokenCommand = (text) => {
    ...
    if (urlMatch) {
        ...
        for (const url of urlMatch) {
            ...
            if (parsed.platform === 'twitter' || parsed.platform === 'farcaster') {
                result.context = parsed;
                break;
            }

            if (!fallbackContext) fallbackContext = parsed;
        }

        if (!result.context && fallbackContext) {
            result.context = fallbackContext;
        }
    }

    // Extract fees (prioritize explicit split format first)
    const textWithoutUrls = text.replace(/https?:\/\/[^\s]+/gi, ' ');
    const splitPercentMatch = textWithoutUrls.match(/(\d+(?:\.\d+)?)\s*%\s*(?:\/|\s|,|and)\s*(\d+(?:\.\d+)?)\s*%/i);
    if (splitPercentMatch) {
        result.fees = parseFees(splitPercentMatch[0]);
    }

    if (!result.fees) {
        const feeMatch = textWithoutUrls.match(/(\d+(?:\.\d+)?)\s*(?:%|percent|bps)/i);
        if (feeMatch) {
    ...
    if (!result.fees) {
        const withFeesMatch = textWithoutUrls.match(/(?:with|fees?:?)\s*(\d+(?:\.\d+)?)\s*(?:%|bps)?/i);
        if (withFeesMatch) {
