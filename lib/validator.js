...
export const validateConfig = (config) => {
    ...
    if (config.fees.type === 'static') {
        ...
        if (!Number.isFinite(clankerFee) || !Number.isFinite(pairedFee)) {
            ...
            addFix('Invalid static fees detected; reset to 2.5% + 2.5%');
        } else {
        ...
        if (totalFee > 500) {
            ...
            config.fees.clankerFee = 250;
            config.fees.pairedFee = 250;
            addFix(`Total fee ${totalFee} bps exceeded protocol cap; reset to 500 bps total`);
        }
        ...
        config.fees = { type: 'static', clankerFee: 250, pairedFee: 250 };
        addFix('Unsupported fee type detected; reset to static 2.5% + 2.5%');
    }
