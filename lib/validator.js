...
const CLANKERWORLD_SOCIAL_PLATFORMS = new Set(['twitter', 'farcaster']);
...
export const validateConfig = (config) => {
    ...
    const allowCustomFeeRange = meta.allowCustomFeeRange === true;
    ...
    if (config.fees.type === 'static') {
        ...
        if (!allowCustomFeeRange && totalFee > MAX_STATIC_TOTAL_FEE_BPS) {
            if (!smartMode) {
                throw new Error(`Static total fee exceeds ${MAX_STATIC_TOTAL_FEE_BPS} bps compatibility limit (set advanced.allowCustomFeeRange=true to override)`);
            }
        ...
        if (!allowCustomFeeRange && Number(config.fees.maxFee) > 500) {
            if (!smartMode) {
                throw new Error('Dynamic max fee exceeds 500 bps compatibility limit (set advanced.allowCustomFeeRange=true to override)');
            }
    ...
    if (CLANKERWORLD_SOCIAL_PLATFORMS.has(platform) && !contextUserId) {
        console.warn(`⚠️  \x1b[33mWarning:\x1b[0m Context user id is empty for ${platform}. Set context.id/contextUserId to improve Clankerworld matching.`);
    }
    if (platform === 'twitter' && contextUserId && !/^\d+$/.test(contextUserId)) {
        console.warn('⚠️  \x1b[33mWarning:\x1b[0m Twitter context.id is non-numeric. Use numeric user id for strongest Clankerworld matching.');
    }
    ...
    if (Array.isArray(config.rewards?.recipients) && config.rewards.recipients.length > 1 && CLANKERWORLD_SOCIAL_PLATFORMS.has(platform)) {
        console.warn(`⚠️  \x1b[33mWarning:\x1b[0m Spoof/split rewards detected. If social context identity does not match deploy provenance, token may not appear in Clankerworld listings.`);
    }
